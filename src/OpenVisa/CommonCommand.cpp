...
/*!
    ...
    \sa         ese
*/
void Object::CommonCommand::setEse(int en)
{
    send("*ESE {}", en);
}
...
{
    return OpenVisa::decode<int>(query("*ESE?"));
}
...
/*!
    ...
    \sa         opc
*/
void Object::CommonCommand::setOpc()
{
    send("*OPC");
}
...
{
    return OpenVisa::decode<bool>(query("*OPC?"));
}
...
{
    return OpenVisa::split(query("*OPT?"), ",");
}
...
/*!
    ...
    \sa         sre
*/
void Object::CommonCommand::setSre(int status)
{
    send("*SRE {}", status);
}
...
{
    return OpenVisa::decode<int>(query("*SRE?"));
}
...
{
    return OpenVisa::decode<StatusByteRegisters>(query("*STB?"));
}
...
{
    return !OpenVisa::decode<int>(query("*TST?"));
}

/*!
    \brief      配置仪器的输出缓冲区，等待所有未决操作完成之后，再通过接口执行任何其他命令。
*/
void Object::CommonCommand::wai()
{
    send("*WAI");
}
...
/*!
    \brief      调用寄存器保存文件 \a i.
*/
void Object::CommonCommand::rcl(int i)
{
    send("*RCL ", i);
}
